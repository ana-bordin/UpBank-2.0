...
using UPBank.Employee.Domain.Entities;
...
namespace UPBank.Employee.Infra.Data.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<Domain.Entities.Employee> AddAsync(Domain.Entities.Employee employee)
        {
            try
            {
                await _context.Connection.ExecuteAsync("INSERT INTO Employee (CPF, Manager, RecordNumber) VALUES (@cpf, @manager, @recordNumber)", new { employee.CPF, employee.Manager, employee.RecordNumber });
        ...
        public async Task<Domain.Entities.Employee?> UpdateAsync(Domain.Entities.Employee entity)
        {
            try
            {
                var rows = await _context.Connection.ExecuteAsync("UPDATE dbo.Employee SET Manager = @Manager WHERE CPF = @CPF", new { entity.CPF, entity.Manager });
        ...
        public async Task<IEnumerable<RequestAccount>> AccountOpeningRequests()
        {
            try
            {
                return await _context.Connection.QueryAsync<RequestAccount>("SELECT * FROM AccountOpeningRequests");
            }
            catch (Exception e)
            {
                _domainNotificationService.Add("Houve um erro ao buscar requições");
                return null;
            }
